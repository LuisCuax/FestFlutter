  ...
  Future<void> _fetchChats() async {
    try {
      ...
      var query = SupabaseClientConfig.instance
          .from('chat_channels')
          .select('*, requests(title), client_profile:client_id(full_name, avatar_url), provider_profile:provider_id(business_name, full_name, avatar_url)')
                  ...
                  : _channels.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final otherProfile = isClient ? item['provider_profile'] : item['client_profile'];
                            final otherName = isClient 
                                ? (otherProfile?['business_name'] ?? otherProfile?['full_name'] ?? 'Proveedor')
                                : (otherProfile?['full_name'] ?? 'Cliente');
                            
                            final initials = otherName.isNotEmpty ? otherName.substring(0, 2).toUpperCase() : '??';
                            final requestTitle = item['requests']?['title'] ?? 'Sin asunto';
